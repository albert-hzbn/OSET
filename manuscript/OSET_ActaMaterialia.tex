...
quantities. \OSET\ is validated against the original Padmanabhan et
al.\ grain-boundary-sliding data, independent literature spanning
...
\textit{Numerical check.} This first-principles elastic result can
...
\textbf{Method~1 (kinematic, exact).} The eigenstrain
...
\textbf{Method~2 (moment tensor).} As an independent check, $\beff$
...
\[
  ...
  \;\Rightarrow\;
  \beff=\frac{\gz\cdot\frac{2\pi W^3}{3}}{\pi W^2}=\frac{2\gz W}{3},
\]
which underestimates Method~1 by 33\%.
\textit{Origin of the discrepancy.} The seismic-moment formula
...
\textbf{Method~3 (Lorentzian normalisation).} The third, weaker check
...
\textit{Physical basis.} A shear traction $\sigma_{13}$ on the
...
\textit{Applicability to the \OSTZ\ $r^{-3}$ field.} Three features
...
non-negligible.
\textit{Why an isotropic approximation is needed.} The exact
...
\textbf{Theorem (dislocation emergence).} A co-planar chain of $N$
...
\textit{Proof.} The cumulative slip $U(x)$ at a field point $x$ is,
...
\begin{equation}
  ...
  \label{eq:Nc}
\end{equation}
For FCC copper ($W=b$, $\gz=0.12$) this gives $N_c\approx8$; for the
...
\textit{Physical argument.} The characteristic activation stress
...
(Frenkel range~\cite{frenkel1926} $G/10$--$G/30$).
...
\begin{equation}
  ...
  \label{eq:V0OSET}
\end{equation}
For Cu: $\tau^*\approx G/17$ vs.\ the Frenkel estimate~\cite{frenkel1926} $G/[2(1-\nu)]\approx0.75G$.
...
(experiment~\cite{nabarro1997}: $\sim10^{-4}G$).
...
$\alpha=\pi\beta_1\gz/3\approx0.13$ (exp/DFT~\cite{woodward2002}: $0.1$--$0.2$).
...
\textit{Physical picture.} When only $N_p$ (rather than the full
...
$\gamma_\text{SF}\propto Gb$ across FCC metals; the full prediction
for Cu, Al, Ni, Ag, and Au, together with a discussion of the larger
discrepancies for Al, Ni, and Ag, is given in the Supporting
Information (Table~S1). The same parameter-free formula applies
unchanged to the basal-plane stacking fault of an HCP metal, with
$G$, $b$ taken as the basal-plane values; the Supporting Information
(Table~S2) extends the comparison to Mg and Ti, finding agreement for
Mg but an underprediction for Ti consistent with the HCP twin-boundary
discrepancy of \S\ref{sec:gb}. No comparable quantitative test is made for BCC metals, which lack a
single well-defined low-energy stacking fault analogous to FCC and
HCP, as discussed in the Supporting Information.
...
(Table~S3) extends this verification to two hexagonal close-packed
...
\textbf{Theorem.} \OSET\ logically entails classical dislocation
...
\textit{Proof of $\Rightarrow$.} Sections~\ref{sec:PN}--\ref{sec:chainStress}
...
\textit{Proof that the converse fails.} It suffices to exhibit
...
Figure~S3 there consolidates the bulk-metallic-glass,
...
Table~S7. Four findings emerge. The
...
dataset. The activation energy $\dF$ accounts for only about a
...
grain-boundary sliding event alike. Microstructural design strategies
already developed for dislocation-mediated plasticity, such as
core-width control through solute segregation, may therefore transfer
in modified form to amorphous and nanocrystalline systems, where no
